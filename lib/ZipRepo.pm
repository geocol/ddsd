...
sub new_from_upstream ($$$;%) {
  ...
  $self->_set_key (join $;, $self->{forced_encoding}//'', $self->{forced_tzoffset}//'');
...
sub read_index ($) {
  ...
  return $self->{upstream_repo}->read_index->then (sub {
    ...
    my $storage = $upix->get_storage_of
        ($upitem, (join '-', @{$self->{key}}), allow_missing => 1,
...
sub lock_index ($) {
  ...
  return $self->{upstream_repo}->lock_index->then (sub {
    ...
    my $storage = $upix->get_storage_of
        ($upitem, (join '-', @{$self->{key}}), prefix => 'archive-',
  ...
  return $self->read_index->then (sub {
    ...
    return Promise->all ([
      ...
      for my $zipped_file (@{$info->{files}}) {
        ...
        ## from those of earlier versions.  In constrast, $file->{rev}
        ## set by |_set_item_file_info| comes from the archive's
        ## repository index file, which can contain a snapshot results
        ## of sniffing generated by some earlier version.  Still the
        ## connections between $file objects and items in the
        ## archive's repository index file are kept through the raw
        ## paths of them.  Also note that any nested archive
        ## repository (i.e. a repository for ZIP file contained in
        ## this ZIP file) would also have its |ddsd ls| |package|
        ## item's |rev| set to the upstream repository's ZIP file's
        ## |rev|.
