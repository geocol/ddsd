...
Test {
  ...
  return $current->prepare (
    ...
    return $current->check_files ([
      ...
      {path => "local/data/foo/files/abc/part-0.ttl", text => "0"},
      {path => "local/data/foo/files/abc/part-f.ttl", text => "f"},
    ]);
  });
} n => 25, name => 'main renamed';
  ...
  return $current->prepare (
    {
      foo => {
        ...
        url => "https://hoge/$key/pack.json",
        files => {
          "part:sparql[file:r:sparql]:1" => {name => "file1"},
        },
      },
    },
    {
        ...
        json => {
          ...
          source => {
            ...
            files => {
              ...
              "part:sparql[file:r:sparql]:2" => {name => "file2"},
            },
          },
        },
      },
    ...
    return $current->check_files ([
      {path => 'local/data/foo/index.json', json => sub {
         ...
         {
           my $item = $json->{items}->{'part:sparql[file:r:sparql]:1'};
           is $item->{files}->{data}, 'files/file1';
           is $item->{rev}->{url}, "https://hoge/$key/sparqlep?query=SELECT%20%2A%20WHERE%20%7B%20%20%3Fs%20%3Fp%20%3Fo%20.%20%20FILTER%20(STRSTARTS(SUBSTR(MD5(STR(%3Fs)),%201,%202),%20%221%22))%20%7D";
           is $item->{type}, 'part';
         }
         {
           my $item = $json->{items}->{'part:sparql[file:r:sparql]:2'};
           is $item->{files}->{data}, 'files/file2';
           is $item->{rev}->{url}, "https://hoge/$key/sparqlep?query=SELECT%20%2A%20WHERE%20%7B%20%20%3Fs%20%3Fp%20%3Fo%20.%20%20FILTER%20(STRSTARTS(SUBSTR(MD5(STR(%3Fs)),%201,%202),%20%222%22))%20%7D";
           is $item->{type}, 'part';
         }
         ok $json->{items}->{'part:sparql[file:r:sparql]:3'};
         ok $json->{items}->{'part:sparql[file:r:sparql]:4'};
