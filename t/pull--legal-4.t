...
Test {
  ...
  return $current->prepare ({
    hoge => {type => 'ckan', url => "https://data.bodik.jp/$key/dataset/$key"},
  }, {
        ...
        source => {
          ...
          files => {
            'file:r:websites.json' => {
              url => "https://data.bodik.jp/websites.json",
            },
          },
        },
      },
    },
    "https://data.bodik.jp/websites.json" => {
      json => [{
        terms_url => "https://data.bodik.jp/$key/license",
        source => {type => 'packref', url => "https://data.bodik.jp/$key/license.json"},
        legal_key => "$key-license",
      }],
    },
    "https://data.bodik.jp/$key/license.json" => {
      json => {
        type => 'packref',
        source => {type => 'files'},
      },
    ...
    return $current->check_files ([
      {path => $current->repo_path ('ckan', "https://data.bodik.jp/$key/dataset/$key").'/index.json', json => sub {
         my $json = shift;
         my $path = shift;
         
         is 0+keys %{$json->{items}}, 2;
         my $file_key = $json->{urls}->{"https://data.bodik.jp/$key/api/action/package_show?id=$key"};
    ...
    return $current->check_files ([
      {path => $current->repo_path ('ckan', "https://data.bodik.jp/$key/dataset/$key").'/index.json', json => sub {
         ...
         is 0+@$lines, 3;
         {
           ...
           is $v->{timestamp}, $current->o ('log1')->{timestamp};
           is $v->{legal_key}, "$key-license";
           is $v->{legal_source_key}, "package:index.html";
           is $v->{legal_source_url}, "https://data.bodik.jp/$key/license";
         }
         {
           my $v = json_bytes2perl $lines->[1];
      ...
      {path => $current->repo_path ('packref', "https://data.bodik.jp/$key/license.json") . '/index.json', json => sub {
         my $json = shift;
         my $path = shift;
         my $file_key = $json->{urls}->{"https://data.bodik.jp/$key/license.json"};
         my $log_path = $path->parent->child
             ($json->{items}->{$file_key}->{files}->{log});
         my $lines = [split /\x0A/, $log_path->slurp];
         is 0+@$lines, 1;
...
Test {
  ...
  return $current->prepare ({
    hoge => {
      type => 'ckan', url => "https://data.bodik.jp/$key/dataset/$key",
      files => {
        package => {
  ...
  return $current->prepare ({
    ...
    "https://data.bodik.jp/$key/api/action/package_show?id=" . $key => {
      text => qq{ {"success": true, "result": {
        "resources": [{"url": "https://data.bodik.jp/$key/abc"}]
      }} },
    },
    "https://data.bodik.jp/$key/abc" => {text => "ABC"},
    "https://data.bodik.jp/$key/dataset/" . $key => {
      text => qq{<a href="/$key/license">オープンデータ利用規約</a>},
    },
  })->then (sub {
    ...
    return $current->prepare (undef, {
      "https://data.bodik.jp/$key/abc" => {text => "ABC2"},
    });
  })->then (sub {
    return $current->run ('pull');
  })->then (sub {
...
Test {
  ...
  return $current->prepare ({
    ...
    "https://data.bodik.jp/websites.json" => {
      json => [{
        terms_url => "javascript:/$key/license",
        source => {type => 'packref', url => "https://data.bodik.jp/$key/license.json"},
        legal_key => "$key-license",
      }],
    },
    ...
    "https://data.bodik.jp/$key/dataset/" . $key => {
      text => qq{<a href="javascript:/$key/license">オープンデータ利用規約</a>},
    },
  })->then (sub {
    return $current->run ('pull');
  })->then (sub {
    ...
    return $current->check_files ([
      {path => $current->repo_path ('ckan', "https://data.bodik.jp/$key/dataset/$key").'/index.json', json => sub {
         ...
         my $file_key = $json->{urls}->{"https://data.bodik.jp/$key/api/action/package_show?id=$key"};
         is $json->{items}->{$file_key}->{files}->{log}, undef;
       }},
      {path => "local/data/hoge/package-ckan.json",
       text => q{ {"success": true, "result": {}} }},
    ]);
  });
} n => 4, name => 'legal url bad 1';
...
Test {
  ...
  return $current->prepare ({
    ...
    "https://data.bodik.jp/websites.json" => {
      json => [{
        terms_url => "https://x:y/$key/license",
        source => {type => 'packref', url => "https://data.bodik.jp/$key/license.json"},
        legal_key => "$key-license",
      }],
    },
    ...
    "https://data.bodik.jp/$key/dataset/" . $key => {
      text => qq{<a href="https://x:y/$key/license">オープンデータ利用規約</a>},
    },
  })->then (sub {
    return $current->run ('pull');
  })->then (sub {
